...
def makeMorphs(theDuration,theFrameRate,theImage1,theImage2,theList1,theList2,theList4,size,theResult, email):
    cv2.namedWindow('morphing')
    cv2.startWindowThread()
    totalImages=int(theDuration*theFrameRate)
    # output an image of size.
    size = theImage1.shape
    fourcc = cv2.VideoWriter_fourcc('X', 'V', 'I', 'D')
    out = cv2.VideoWriter('./entertainment_results/' + email + '_output.avi', fourcc, 10, (size[1], size[0]))
    ...
    for j in range(0,totalImages):
        ...
        text = "Morphing your face..."
        textsize = cv2.getTextSize(text, font, 1, 2)[0]
        cv2.putText(frame, text, (int((1280-textsize[0])/2), 30),
            font, 1, (0,0,255), 2)
        cv2.imshow('morphing', frame)
        cv2.waitKey(2)
        out.write(np.uint8(imgMorph))
